...
const roleConfig = {
  sales: {
    ...
    description:
      'Close more deals with intelligent lead management and automated follow-ups',
    features: [
  ...
  manager: {
    ...
    title: 'Sales Manager',
    description:
      'Drive team performance with comprehensive analytics and coaching tools',
    features: [
...
export default function FeatureCard({
  ...
  return (
    <Card className='relative overflow-hidden border-2 transition-all duration-300 hover:shadow-lg hover:scale-105'>
      <div
        className={`absolute top-0 left-0 h-1 w-full ${config.color === 'blue' ? 'bg-blue-500' : config.color === 'green' ? 'bg-green-500' : 'bg-purple-500'}`}
      ></div>

      <CardHeader className='pb-4'>
        <div
          className={`inline-flex h-12 w-12 items-center justify-center rounded-lg ${colorClasses[config.color as keyof typeof colorClasses]} mb-4`}
        >
          {config.icon}
        </div>
        ...
        <ul className='space-y-3'>
          {config.features.map((feature, index) => (
            <li key={index} className='flex items-start gap-3'>
              <div
                className={`mt-1 h-2 w-2 flex-shrink-0 rounded-full ${config.color === 'blue' ? 'bg-blue-500' : config.color === 'green' ? 'bg-green-500' : 'bg-purple-500'}`}
              ></div>
